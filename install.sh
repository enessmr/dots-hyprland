...
startask () {
  ...
  printf '            2. does not handle system-level/hardware stuff like Nvidia drivers, but I think i added variables.\n'
  printf "\e[31m"
...
printf "\e[36m[0: Copying the wallpaper for the REQUIRED step... (required, also can use Ctrl+Super+T for wallpaper sel, please check cheatsheet via super+shift+i)\e[0m\n"
cp -r ./.woolpaper $HOME/
...
detect_nvidia_gpu() {
    if lspci | grep -i "nvidia"; then
        printf "${cyan}[0: NVIDIA GPU detected!${reset}\n"
        return 0
    else
        printf "${cyan}[0: No NVIDIA GPU detected.${reset}\n"
        return 1
    fi
}
...
install_nvidia_driver() {
    printf "${cyan}[0: Installing NVIDIA...${reset}\n"

    # Install appropriate NVIDIA driver
    sudo pacman -S nvidia nvidia-utils egl-wayland --noconfirm

    printf "${cyan}[0: NVIDIA Installation Complete.${reset}\n"
}

# Main Execution
printf "${cyan}[0: Starting NVIDIA Detection and Installation Script...${reset}\n"

# Detect NVIDIA GPU and install the driver if found
if detect_nvidia_gpu; then
    install_nvidia_driver
else
    printf "${cyan}[0: Skipping NVIDIA installation.${reset}\n"
fi
printf "${cyan}[0:NVIDIA done.${reset}\n"
#####################################################################################
